...
(function (global) {
  ...
  const KNOWLEDGE = [
    ...
    },
    {
      id: 'raffle_fairness',
      topic: 'raffleTicket',
      keywords: ['抽獎公平', '抽奖公平', '抽獎機制', '抽奖机制', '公平性', '公正', '螢幕錄影', '录屏'],
      match(n) {
        return (/公平|公正|作弊|黑箱|錄影|录屏/.test(n) && /抽獎|抽奖|抽獎券|抽奖券|摸彩/.test(n))
          || /抽獎機制|抽奖机制/.test(n)
          || (/抽獎|抽奖/.test(n) && /公平|公正|知道.*公/.test(n));
      },
      get() {
        return formatAnswer({
          conclusion: '雙月抽獎採公開、可驗證方式進行：以台灣彩券開獎號碼作為種子，透過固定公式產生中獎名單，10 月場與 12 月場皆全程螢幕錄影並公布。',
          details: [
            '開獎前會公告使用的彩券期別與號碼；因無人可預測開獎結果，無法事先操控名單。',
            '抽獎券依任務完成累積，券越多中獎機會越高；同一人最多中獎一次。',
            '開獎過程全程螢幕錄影，中獎名單亦會在活動網頁公開公布。',
            '每次抽完後，抽獎券會重新計算。',
          ],
          nextStep: step('drawTime'),
